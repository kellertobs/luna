...
D        =  1;                   % chamber depth [m]
L        =  1;                   % chamber width [m]
N        =  1;                   % number of grid points in z-direction (incl. 2 ghosts)
...
tend     =  25*yr;               % end time for simulation [s]
dt       =  yr/50;               % initial time step [s]
dtmax    =  yr/50;               % maximum time step [s]
...
dc       =  [0,0,0,0].*1e-4;     % amplitude of random noise [wt SiO2]
...
bndmode  =  1;                   % boundary assimilation mode (0 = none; 1 = top only; 2 = bot only; 3 = top/bot;)
bndinit  =  0;                   % switch on (1) to initialise with already established boundary layers
bnd_w    =  10*D;                % boundary layer thickness for assimilation [m]
...
etacntr  =  1e+20;               % maximum viscosity contrast
